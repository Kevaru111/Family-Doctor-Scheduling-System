...
class TvarkarascioSistema:
    ...
    def priimti_pacienta(self):
        ...
        if not artimiausia.ar_uzimtas():
            print("Pacientas: LAISVAS LAIKAS")
            print("="*50)
            print("\n✓ Laisvas laiko langelis pažymėtas kaip priimtas")
        ...
        print("\n" + "─"*50)
        print("PACIENTO APSILANKYMŲ ISTORIJA")
        print("─"*50)
        
        if len(artimiausia.pacientas.istorija) == 0:
            print("Pirmas apsilankymas istorija tuščia")
        else:
            for i, irasas in enumerate(artimiausia.pacientas.istorija, 1):
                print(f"{i}. {irasas['data']}")
                print(f"   Simptomai: {irasas['simptomai']}")
                print(f"   Diagnozė: {irasas['diagnoze']}")
        
        print("─"*50 + "\n")
        
        while True:
            diagnoze = input("Įveskite diagnozę: ").strip()
            if diagnoze:
                break
        ...
        self.atnaujinti_paciento_json(artimiausia.pacientas)
    ...
    def atnaujinti_paciento_json(self, pacientas):
        with open('Pacientai.json', 'r', encoding='utf-8') as f:
            pacientai_data = json.load(f)
        
        for p in pacientai_data:
            if p['asmens_kodas'] == pacientas.asmens_kodas:
                p['istorija'] = list(pacientas.istorija)
                break
        
        with open('Pacientai.json', 'w', encoding='utf-8') as f:
            json.dump(pacientai_data, f, ensure_ascii=False, indent=2)
